...
public class MiniWeapon_Centavo: BaseWeapon
{

    [Header("BASE STATS")]
    //[SerializeField]
    private float baseCooldown = 5f; // same for either
    private float baseDamage = 0.1f;

    private float baseKnockbackPower = 15f;
    private float baseKnockbackDuration = .5f;

    private float baseProjectileSpeed = 8f;
    ...
    public float finalKnockbackDuration;
    ...
    private void DoTick() // to be called per update
    {
        ...
        if (cooldownTimer >= finalCooldown)
        {
            ...
            GameObject closestEnemy = DetectCentavoTarget();
    ...
    private void DoScaleStats()
    {
        ...
        finalKnockbackDuration =
            baseKnockbackDuration
    ...
    private void DoAttack(Vector2 _originPos, Vector2 _targetPos)
    {
        ...
        attackInstance.GetComponent<CentavoProjectile>().Initialize(this);
    ...
    private GameObject DetectCentavoTarget()
    {
